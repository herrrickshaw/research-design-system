#!/usr/bin/env python3
"""Render the Lok Sabha Question Registry as a self-contained Blogger post.
...
rows = json.load(open('/Users/umashankar/digital-twin-for-ipa/data/registers/ls_pq_registry.json'))
...
# [iso, [schemeIdx], minIdx, lok, qtype(0=UNSTARRED,1=STARRED), qno, subject, pdf, members]
R = [[r['iso'], sorted(si[s] for s in r['schemes']), mi[r['ministry']], r['lok'],
      1 if r['qtype'] == 'STARRED' else 0, r['qno'], r['subject'],
...
starred = sum(1 for r in rows if r['qtype'] == 'STARRED')
...
<h1>Lok Sabha Question Registry</h1>
<p class="dek"><strong>Every captured Lok Sabha question</strong> on investment policies, schemes
and incentives, across the 17th and 18th Lok Sabha ({first} &rarr; {last}) &mdash; {len(R):,} at
this build &mdash; each mapped to its
...
<p class="dek">This registry is the parliamentary companion to the
<a href="https://masaladeutsch.blogspot.com/2026/08/quarterly-reportage-4034-scheme.html"
style="color:var(--acc)">Quarterly Reportage</a>: where the reportage tracks what government
<em>announced</em>, this tracks what Parliament <em>asked</em> &mdash; and the ministry's
answer of record. Questions are classified by the same scheme-keyword map, so the two views
filter identically. Use it to find the answer, then read the answer. The underlying data
...
<p class="note">
<strong>Method.</strong> Harvested from sansad.in&#39;s Lok Sabha Q&amp;A index by subject
keyword (17th LS: 2019&ndash;2024; 18th LS: 2024&ndash;today), then classified against the
...
<code>scripts/build_pq_registry.py</code> in
...
<strong>Known limits, stated rather than hidden.</strong> The sansad.in index matches
...
assistance from the public sansad.in Lok Sabha Q&amp;A index linked above. AI-generated
...
function render(){{
  ...
  for(var i=0;i<n;i++){{
    ...
    out.push('<div class="row"><div class="top"><span class="date">'+r[0]+'</span>'+chips+
      ...
      '<span class="qn">LS'+r[3]+' &middot; '+(r[4]?'<span class="star">&#9733; STARRED</span>':'Unstarred')+
...
open('/private/tmp/claude-501/-Users-umashankar/e50aaf59-8643-4790-a7e8-d74da5d3e4a2/scratchpad/pq_post.html',
